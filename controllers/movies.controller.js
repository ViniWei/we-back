...
export async function get(req, res) {
    ...
    if (!movie) {
        return res
            .status(404)
            .send(
                errorHelper.buildStandardResponse("Movie not found.", "movie-not-found")
            );
    }
...
export async function create(req, res) {
    ...
    !req.body.api_id
        ...
        return res
            ...
            .json(
                errorHelper.buildStandardResponse(
                    "Missing required fields: title, synopsis, poster_path and api_id.",
                    "missing-fields"
                )
            );
    }
...
export async function getListsWithMoviesByGroup(req, res) {
    const { group_id } = req.session.user;

    try {
        const result = await moviesRepository.getListsWithMoviesByGroup(group_id);
        ...
        return res
            ...
            .send(
                errorHelper.buildStandardResponse(
                    "Error while fetching all lists and movies by group.",
                    "error-db-get-lists-and-movies-group",
                    error
                )
            );
    }
