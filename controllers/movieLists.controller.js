...
export async function create(req, res) {
    if (!req.body.name || !req.body.group_id) {
        return res
            .status(400)
            .json(
                errorHelper.buildStandardResponse(
                    "Missing required fields: name and group_id.",
                    "missing-fields"
                )
            );
    }
...
export async function addToList(req, res) {
    ...
    if (!list_id || !movie_id) {
        return res
            ...
            .json(
                errorHelper.buildStandardResponse(
                    "Missing required fields: list_id and movie_id.",
                    "missing-fields"
                )
            );
    }
...
export async function deleteList(req, res) {
    ...
    try {
        ...
        return res
            ...
            .send(
                errorHelper.buildStandardResponse(
                    "Error while removing movie list.",
                    "error-db-remove-movie-list",
                    error
                )
            );
    }
