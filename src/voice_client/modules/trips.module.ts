...
function cleanCity(name: string | null): string | null {
  ...
  city = city
    ...
    .replace(/\b(com\s+orcamento.*|com\s+orçamento.*|r\$.*|no\s+dia.*|do\s+dia.*|entre\s+os?\s+dias?.*|ate\s+o\s+dia.*|até\s+o\s+dia.*)/gi, "")
    .replace(/\s+/g, " ")
    .trim();
...
function extractBudget(text: string): number | null {
  ...
  if (match) {
    ...
    raw = raw.replace(/\s/g, "").replace(/\.(?=\d{3}\b)/g, "").replace(",", ".");
...
async function fetchTrips(token: string) {
  ...
  if (res.status !== 200 || !Array.isArray(res.data)) throw new Error("Error fetching trips.");
  return res.data;
}

async function fetchPlannedTrips(token: string) {
  const res = await fetchTrips(token);
  const planned = res.filter((t: any) => (t.status || "").toLowerCase().includes("plan"));
  if (planned.length === 0) return { message: "No planned trips found.", trips: [] };
  return {
...
function extractTripData(text: string): any | null {
  ...
  if (/(adicionar|marcar|programar|criar|planejar|viagem\s+para)/i.test(t)) {
    const cityMatch = t.match(/viagem\s+(?:para|pra|pro|em)\s+([a-zçãéêíóúà\s-]+)/i);
...
async function execute(text: string, token?: string) {
  try {
    const data = extractTripData(text);
    if (!data) return { error: "Unable to process the command." };
    if (data.__action__ === "view") return await fetchPlannedTrips(token || "");
