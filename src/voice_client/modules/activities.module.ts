...
function decodeToken(token: string): { user_id: number | null; group_id: number | null } {
  try {
    const payload: any = jwt.decode(token);
    const uid = payload?.id ?? payload?.userId ?? payload?.user_id ?? null;
    const gid = payload?.groupId ?? payload?.group_id ?? null;
    return { user_id: uid != null ? Number(uid) : null, group_id: gid != null ? Number(gid) : null };
  } catch {
...
function parseDateParts(dayStr: string, monthWord: string | null, yearStr: string | null, fallbackMonth: number | null, fallbackYear: number): Date | null {
...
function extractExplicitDate(t: string): { date: Date | null; start: number; end: number } {
  const now = new Date();
  const mNum = t.match(/(\d{1,2})[\/\-](\d{1,2})(?:[\/\-](\d{2,4}))?/);
  if (mNum) {
    const d = parseDateParts(mNum[1], null, mNum[3] || null, parseInt(mNum[2]), now.getFullYear());
    return { date: d, start: mNum.index!, end: mNum.index! + mNum[0].length };
  }
  const mTxt = t.match(/(\d{1,2}|primeiro)\s+de\s+([a-zç]+)(?:\s+de\s+(\d{4}))?/);
  if (mTxt) {
  ...
  const wd: Record<string, number> = { domingo: 0, segunda: 1, terca: 2, terça: 2, quarta: 3, quinta: 4, sexta: 5, sabado: 6, sábado: 6 };
  ...
  if (t.includes("amanha") || t.includes("amanhã")) {
    ...
    const s = i >= 0 ? i : 0;
    return { date: d, start: s, end: s + (t.includes("amanhã") ? "amanhã".length : "amanha".length) };
  }
  if (t.includes("depois de amanha") || t.includes("depois de amanhã")) {
    const d = new Date(); d.setDate(d.getDate() + 2);
    const i = t.indexOf("depois de amanha") >= 0 ? t.indexOf("depois de amanha") : t.indexOf("depois de amanhã");
    const s = i >= 0 ? i : 0;
    const len = t.includes("depois de amanhã") ? "depois de amanhã".length : "depois de amanha".length;
...
function extractExplicitTime(t: string): { time: { hour: number; minute: number } | null; start: number; end: number } {
  ...
  return { time: { hour, minute }, start: m.index!, end: m.index! + m[0].length };
}

function extractLocationSpan(t: string): { location: string | null; start: number; end: number } {
  ...
  for (const p of preps) {
    ...
    if (m) {
      ...
      if (sub) return { location: toTitleCase(sub), start: m.index!, end: m.index! + m[0].length };
    }
  }
...
function extractEventNameDynamic(original: string, t: string): string {
  const verbs = ["marcar", "agendar", "criar", "adicionar", "planejar", "programar", "registrar"];
  let event = "";
  const verb = verbs.find(v => t.includes(v));
  if (verb) {
    const regex = new RegExp(`\\b${verb}\\b(?:\\s+(?:um|uma|o|a)\\b)?\\s*(.+)`, "i");
...
function extractActivityData(text: string): any | null {
  ...
  if (!dSpan.date) {
    return { error: "Nenhuma data reconhecida. Especifique um dia ou data para criar a atividade." };
  }
  ...
  const description = `${eventName} em ${location} (${date.toLocaleString("pt-BR")})`;
...
async function fetchActivities(token: string, groupId: number, mode: "all" | "upcoming") {
  const endpoint = mode === "all" ? `/activities/group/${groupId}` : `/activities/group/${groupId}/upcoming`;
  const res = await axios.get(`${BACKEND_URL}${endpoint}`, { headers: { Authorization: `Bearer ${token}` } });
  return res.data;
}

async function sendActivityToBackend(activity: any, token: string) {
  const res = await axios.post(`${BACKEND_URL}/activities`, activity, { headers: { Authorization: `Bearer ${token}` } });
  if (res.status === 201) return { message: `Activity created: ${activity.event_name}` };
  return { error: `Failed to create: ${res.statusText}` };
}

export async function execute(text: string, token: string) {
  const data = extractActivityData(text);
  if (!data) return { message: "Nenhuma intenção reconhecida a partir do comando de voz." };

  if (data.error) return { error: data.error };

  const { user_id, group_id } = decodeToken(token);
  if (data.__action__ === "view_all") return await fetchActivities(token, group_id as number, "all");
  ...
  if (!payload.group_id || !payload.event_name || !payload.date || !payload.location || !payload.created_by) {
